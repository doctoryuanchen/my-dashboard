...
# 🎯 【完美對齊版】獎勵看板（欄位完全對應你的 Google Sheet）
    if df_reward is not None and not df_reward.empty:
        ...
        # 🎯 這裡已經精準改成你圖上的欄位名稱：『獎勵』與『所需金幣』
        if "所需金幣" in df_reward_plot.columns and "獎勵" in df_reward_plot.columns:
            # 確保數字轉換完全正確
            df_reward_plot["所需金幣"] = pd.to_numeric(df_reward_plot["所需金幣"], errors="coerce").fillna(0)
            
            # 帶入剛才由 Python 自己算出的最新累積總金幣
            df_reward_plot["目前累積"] = latest_total_gold
            
            # 計算還差多少（所需金幣 - 目前累積）
            df_reward_plot["還差多少"] = df_reward_plot["所需金幣"] - df_reward_plot["目前累積"]
            ...
            st.warning("⚠️ 找不到『所需金幣』或『獎勵』欄位。")
            st.write("🔍 目前實際抓到的欄位有：", list(df_reward_plot.columns))
